...
pub(super) fn main(args: &Args) -> Result<()> {
    // Generate `fortitude help`.
    let command_help = trim_lines(&help_text());

    // Generate `fortitude help check`.
...
fn help_text() -> String {
    cli::Cli::command().term_width(79).render_help().to_string()
}

/// Returns the output of a given subcommand (e.g., `fortitude help check`).
fn subcommand_help_text(subcommand: &str) -> Result<String> {
    let mut cmd = cli::Cli::command().term_width(79);

    // The build call is necessary for the help output to contain `Usage: fortitude
