            ...
            let settings = match settings {
                ...
                Err(settings) => {
                    self.client.show_error_message(
                        "Fortitude received invalid settings from the editor. Refer to the logs for more information."
                    );
                    settings
                }
            };
...
/// Contains the resolved values of 'editor settings' - Fortitude configuration for the linter/formatter that was passed in via
...
pub(crate) struct EditorSettings {
    ...
    pub(super) check_preview: Option<bool>,
...
pub(crate) enum ResolvedConfigurationError {
    ...
    #[error(transparent)]
    InvalidFortitudeSchema(#[from] toml::de::Error),
}
