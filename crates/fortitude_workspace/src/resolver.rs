...
pub struct Resolver<'a> {
    fortitude_config: &'a FortitudeConfig,
...
impl<'a> Resolver<'a> {
    /// Create a new [`Resolver`] for the given [`FortitudeConfig`].
    pub fn new(fortitude_config: &'a FortitudeConfig) -> Self {
        Self {
            fortitude_config,
            settings: Vec::new(),
            router: Router::new(),
    ...
    /// Return the [`Settings`] from the [`FortitudeConfig`].
    #[inline]
    ...
    pub fn resolve(&self, path: &Path) -> &Settings {
        match self.fortitude_config.strategy {
            FortconfigDiscoveryStrategy::Fixed => &self.fortitude_config.settings,
            FortconfigDiscoveryStrategy::Hierarchical => self
                .router
...
mod tests {
    ...
    use crate::resolver::{
        ConfigurationOrigin, ConfigurationTransformer, FortconfigDiscoveryStrategy,
        FortitudeConfig, ResolvedFile, Resolver, fortran_files_in_path, is_file_excluded,
        match_exclusion, resolve_root_settings,
    };
    ...
    fn rooted_exclusion() -> Result<()> {
        ...
        let fortitude_config = FortitudeConfig::new(
