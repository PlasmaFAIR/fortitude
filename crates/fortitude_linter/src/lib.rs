...
use rules::style::whitespace::{TrailingWhitespace, MissingNewlineAtEndOfFile};
...
pub(crate) fn check_path(
    ...
    if rules.enabled(Rule::MissingNewlineAtEndOfFile) && let Some(violation) = MissingNewlineAtEndOfFile::check(file) {
        violations.push(violation);
    }
