...
impl Violation for MagicNumberInArraySize {
    ...
    fn message(&self) -> String {
        let Self { value } = self;
        format!("Magic number `{value}` in array size")
    }

    fn fix_title(&self) -> Option<String> {
        Some("Replace with named `parameter`".to_string())
    }
}
...
impl AstRule for MagicNumberInArraySize {
    fn check(context: &CheckContext, node: &Node) -> Option<Vec<Diagnostic>> {
        ...
        let size = if node.kind_id() == kind!("type_qualifier") {
        ...
        let violations: Vec<_> = size
            ...
            .filter_map(|child| match child.kind_id() {
                // Need to return a vec here to match next arm
                kind!("number_literal") => Some(vec![child]),
                // This is `X:Y`, pull out the lower and upper bound separately
                kind!("extent_specifier") => Some(
                    child
                        .named_children(&mut child.walk())
                        .filter(|extent| extent.kind_id() == kind!("number_literal"))
                        .collect_vec(),
                ),
                _ => None,
            })
