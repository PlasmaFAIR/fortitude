...
pub(super) mod junit;
pub(super) mod pylint;
pub(super) mod rdjson;
pub(super) mod sarif;
...
use crate::text_helpers::unprintable_replacement;

use super::{Annotation, Diagnostic, DisplayDiagnosticConfig, SubDiagnostic};
...
pub fn render_diagnostics(
    ...
    let value = DisplayDiagnostics::new(&config, diagnostics);
    write!(writer, "{value}")
}
...
impl<'a> DisplayDiagnostic<'a> {
    pub(crate) fn new(
        ...
        DisplayDiagnostic { config, diag }
    }
}

impl std::fmt::Display for DisplayDiagnostic<'_> {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        DisplayDiagnostics::new(self.config, std::slice::from_ref(self.diag)).fmt(f)
    }
}
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self.config.format {
            OutputFormat::Concise => {
                concise::ConciseRenderer::new(self.config).render(f, self.diagnostics)?;
            }
            OutputFormat::Full => {
...
impl<'a> Resolved<'a> {
    ...
    fn new(diag: &'a Diagnostic, config: &DisplayDiagnosticConfig) -> Resolved<'a> {
        let mut diagnostics = vec![];
        diagnostics.push(ResolvedDiagnostic::from_diagnostic(config, diag));
        for sub in &diag.inner.subs {
            diagnostics.push(ResolvedDiagnostic::from_sub_diagnostic(sub));
        }
...
impl<'a> ResolvedDiagnostic<'a> {
    ...
    fn from_diagnostic(
        ...
        let annotations: Vec<_> = diag
            ...
            .iter()
            .filter_map(|ann| {
                // Inlined relativize_path in order to get a &str rather than String
                let path = Path::new(ann.span.file().name());
                let path = if let Ok(path) =
                    path.strip_prefix(path_absolutize::path_dedot::CWD.as_path())
                {
                    path
                } else {
                    path
                ...
                .to_str()
                .unwrap_or_else(|| ann.span.file().name());
                ResolvedAnnotation::new(path, ann.span.file(), ann)
            })
            .collect();
    ...
    fn from_sub_diagnostic(diag: &'a SubDiagnostic) -> ResolvedDiagnostic<'a> {
        let annotations: Vec<_> = diag
            .inner
            .annotations
            .iter()
            .filter_map(|ann| {
                let path = ann.span.file.name();
                ResolvedAnnotation::new(path, &ann.span.file, ann)
            })
            .collect();
        ResolvedDiagnostic {
        ...
        for (path, anns) in ann_by_path {
            ...
            for ann in anns {
                ...
                let prev_context_ends = context_after(
                    &prev.diagnostic_source.to_source_code(),
                    merge_window,
                    prev.line_end,
                )
                .get();
                let this_context_begins = context_before(
                    &ann.diagnostic_source.to_source_code(),
                    merge_window,
                    ann.line_start,
                )
                .get();
...
impl<'a> ResolvedAnnotation<'a> {
    ...
    fn new(
        ...
        let (range, line_start, line_end) = match ann.span.range() {
            ...
            Some(range) => {
                let source = diagnostic_source.to_source_code();
...
impl<'r> RenderableSnippet<'r> {
    ...
    fn new<'a>(context: usize, anns: &'a [&'r ResolvedAnnotation<'r>]) -> RenderableSnippet<'r> {
        ...
        let line_start = context_before(&source, context, content_start_index);

        let content_end_index = anns.iter().map(|ann| ann.line_end).max().unwrap();
        let line_end = context_after(&source, context, content_end_index);
...
";
    ...
    fn basic() {
        ...
        insta::assert_snapshot!(
            env.render(&diag),
            @"
        warning[stable-test-rule]: main diagnostic message
         --> animals:5:1
          |
        3 | canary
        4 | dog
        ...
        insta::assert_snapshot!(
            env.render(&diag),
            @"
        info[stable-test-rule]: main diagnostic message
         --> animals:5:1
          |
        3 | canary
        4 | dog
    ...
    impl TestEnvironment {
        ...
        pub(super) fn new() -> TestEnvironment {
            let mut env = TestEnvironment {
                db: HashMap::default(),
                config: DisplayDiagnosticConfig::new(),
            };
        ...
        pub(super) fn format(&mut self, format: OutputFormat) {
            let config = self.config.clone();
        ...
        pub(super) fn add(&mut self, path: &str, contents: &str) {
            let file = SourceFileBuilder::new(path, contents).finish();
            self.db.insert(path.to_string(), file);
        }
        ...
        pub(super) fn span(
            ...
            let file = span.file();
            let source = file.to_source_code();
        ...
        pub(super) fn path(&self, path: &str) -> Span {
            Span::from(self.db.get(path).unwrap().clone())
        }
        ...
        pub(super) fn builder(
            ...
            let diag = Diagnostic::new(DiagnosticId::Lint(rule), severity, message);
        ...
        pub(super) fn render(&self, diag: &Diagnostic) -> String {
            diag.display(&self.config).to_string()
        }
        ...
        pub(super) fn render_diagnostics(&self, diagnostics: &[Diagnostic]) -> String {
            DisplayDiagnostics::new(&self.config, diagnostics).to_string()
        }
    }
    ...
    pub(crate) fn create_diagnostics(format: OutputFormat) -> (TestEnvironment, Vec<Diagnostic>) {
        let mut env = TestEnvironment::new();
        env.add(
            "test.f90",
            r#"module test
...
"#,
        ...
        env.add("star_kind.f90", r"integer*4 foo; end");
        env.format(format);
        ...
        let diagnostics = vec![
            ...
            .documentation_url("https://fortitude.readthedocs.io/rules/superfluous-implicit-none")
            .build(),
            env.builder(
            ...
            .documentation_url("https://fortitude.readthedocs.io/rules/unnamed-end-statement")
            .sub(|env| {
                // This is a bit of a nonsense, just testing these additional annotations
                env.sub_builder(SubDiagnosticSeverity::Info, "Name from here")
                    .primary("test.f90", "5:13", "5:16", "`foo` is defined here")
                    .secondary("test.f90", "7:16", "7:16", "insert name here")
                    .build()
            })
            .build(),
            env.builder(
            ...
            .primary("star_kind.f90", "1:7", "1:8", "")
            .secondary_code("PORT021")
            .documentation_url("https://fortitude.readthedocs.io/rules/star-kind")
            .build(),
        ];
