        ...
        let output = SarifOutput {
            ...
            runs: [SarifRun {
                tool: SarifTool {
                    driver: SarifDriver {
                        name: "fortitude",
                        information_uri: "https://github.com/PlasmaFAIR/fortitude",
                        rules,
...
impl<'a> From<(&'a str, SarifLevel)> for SarifRule<'a> {
    fn from(code_and_level: (&'a str, SarifLevel)) -> Self {
        ...
        let (linter, suffix) = Category::parse_code(code).unwrap();
        let rule = linter
            .all_rules()
...
impl From<Severity> for SarifLevel {
    fn from(value: Severity) -> Self {
        match value {
            Severity::Info | Severity::None => SarifLevel::Note,
...
impl<'a> SarifResult<'a> {
    ...
    fn fix(diagnostic: &'a Diagnostic, uri: &str) -> Option<SarifFix> {
        ...
        let Some(source_file) = diagnostic.source_file() else {
            debug_assert!(
                false,
                "Omitting the fix for diagnostic with id `{}` because the source file is missing. This is a bug in Fortitude, please report an issue.",
                diagnostic.id()
            );
    ...
    fn uri(diagnostic: &Diagnostic) -> Result<String> {
        let path = normalize_path(&*diagnostic.expect_filename());
...
#[cfg(test)]
mod tests {
    use crate::diagnostics::{OutputFormat, message::tests::create_diagnostics};

    fn get_output() -> String {
        let (env, diagnostics) = create_diagnostics(OutputFormat::Sarif);
